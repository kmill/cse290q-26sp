...
[...]
  ...
  - "meta tactics" for higher-level automated reasoning
    - the simplification engine
        ...
        confluent rewrite system (Church-Rosser)
    - the grind engine
    - `bv_decide` (using the SAT solver CaDiCaL)
-/
example (n : UInt32) (h : n > 0) : (n &&& (n - 1)) < n := by
  bv_decide
...
- Compiler
  ...
  - Optimizations
  - Transformations from IR to C code
- Kernel
...
4. Kernel typechecking.
   ...
   carefully reviewed.
   There are also independent kernels. https://github.com/leanprover/lean-kernel-arena
...
#eval repr (.option1 (20 + 2) : MyType)

#print instReprMyType.repr
...
#check let x := 2; (id rfl : x = 2)
#check (fun x => (id rfl : x = 2)) 2 -- application type mismatch

-- In another world...
-- (fun (x : isDefeqTo 2) => (id rfl : x = 2)) 2
...
(There are also universe level metavariables.)
-/
