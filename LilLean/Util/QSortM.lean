...
/-!
# Quicksort with a monadic comparison function

Adapted from `Init.Data.Array.QSort`.
-/
...
/--
Internal implementation of `Array.qsortM`.

`qpartitionM as lt lo hi hlo hhi` returns a pair `(⟨m, h₁, h₂⟩, as')` where
...
-/
def qpartitionM {n} (as : Vector α n) (lt : α → α → m Bool) (lo hi : Nat) (w : lo ≤ hi := by omega)
    (hlo : lo < n := by omega) (hhi : hi < n := by omega) : m ({m : Nat // lo ≤ m ∧ m ≤ hi} × Vector α n) := do
  let mid := (lo + hi) / 2
  ...
  let rec loop (as : Vector α n) (i k : Nat)
      ...
      m ({m : Nat // lo ≤ m ∧ m ≤ hi} × Vector α n) := do
    if h : k < hi then
      ...
      return (⟨i, ilo, by omega⟩, as.swap i hi)
  loop as lo lo

/--
In-place quicksort.

`qsortM as lt lo hi` sorts the subarray `as[lo...=hi]` in-place using `lt` to compare elements.
-/
@[inline] def qsortM (as : Array α) (lt : α → α → m Bool)
    (lo := 0) (hi := as.size - 1) : m (Array α) := do
  let rec @[specialize] sort {n} (as : Vector α n) (lo hi : Nat) (w : lo ≤ hi := by omega)
      (hlo : lo < n := by omega) (hhi : hi < n := by omega) : m (Vector α n) := do
    if h₁ : lo < hi then
      let ⟨⟨mid, hmid⟩, as⟩ ← qpartitionM as lt lo hi
      if h₂ : mid ≥ hi then
  ...
  if h : as.size = 0 then
    ...
    return (← sort as.toVector lo hi) |>.toArray
